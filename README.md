...
* 支持分片模式上传（适合用于上传超大文件），115 要求分片按顺序上传，因此单个文件的分片为串行上传
* 支持多个上传任务同时进行，可并行上传多个文件
* 支持显示上传进度条
...
`fake115uploader -m 文件` 先尝试用秒传模式上传文件，失败后改用分片模式上传（适合用于上传超大文件）。默认根据文件大小自动分片，每片128MB。可以设置fake115uploader.json的partSizeMB或者用 `-part-size 分片大小` 参数指定分片大小（单位MB，范围为1到5120）；设置fake115uploader.json的partsNum或者用 `-parts-num 分片数量` 参数指定上传文件的分片数量（数量范围为1到10000，设置后忽略分片大小）。

注意：115 的上传协议要求分片按顺序上传（OSS 才会计算整个文件的 SHA1 用于服务端校验），因此单个文件内无法并行上传分片，`-parallel-parts` 参数已无效。要提升上传吞吐量，请用 `-concurrent-uploads` 增加同时上传的文件数。
...
运行时加上参数 `-log-file 文件` 将日志同时保存到指定文件。终端里进度条的重绘会截断日志行，需要完整日志（例如排查问题时）请使用此参数。

终端里的进度条是 docker pull 风格：每个上传任务固定占一行，同一个文件先显示 `校验 文件名`（正在计算文件的 SHA1，速度是磁盘读取速度），校验完成后同一行被 `上传 文件名`（正在上传到 OSS，速度是上传网速，可据此判断上传是否卡死需要人工介入）覆盖，上传完成后该行定格为 100% 的进度条（不显示时间）。小于 1MB 的文件不显示进度条，也不打扰已有的进度显示，只在最终汇总中体现。上传过程中终端只显示进度条和错误信息，成功与否以最后的汇总为准（需要每一步的完整日志请用 `-log-file`）。
