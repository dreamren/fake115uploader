...
终端里的进度条是 docker pull 风格：每个上传任务固定占一行，只显示 `上传 文件名`（正在上传到 OSS，速度是上传网速，可据此判断上传是否卡死需要人工介入）。本地 SHA1 校验很快，不显示进度；小于 1MB 的文件也不显示进度条，两者都只在最终汇总中体现。上传过程中终端只显示进度条和错误信息，成功与否以最后的汇总为准（需要每一步的完整日志请用 `-log-file`）。
